...
const WelcomeForm = ({
  ...
  return <div className="min-h-screen flex items-center justify-center px-4 py-8 relative overflow-hidden">
      ...
      {/* Floating emojis */}
      {floatingEmojis.map((emoji, i) => <motion.span key={i} animate={{
      y: [-20, 20, -20],
      x: [-10, 10, -10],
      rotate: [-10, 10, -10]
    }} transition={{
      ...
      repeat: Infinity,
      delay: i * 0.3
    }} className="absolute text-2xl sm:text-4xl opacity-40 hidden sm:block" style={{
      left: `${5 + i * 12 % 90}%`,
      top: `${10 + i * 13 % 80}%`
    }}>
          {emoji}
      ...
      damping: 20
        ...
        <div className="mt-6 mb-6 sm:mb-8">
          ...
          delay: 0.3
            ...
            {["🎊", "🎉", "🎊"].map((emoji, i) => <motion.span key={i} animate={{
            scale: [1, 1.2, 1],
            rotate: [0, 10, -10, 0]
          }} transition={{
            duration: 2,
          ...
          delay: 0.2
            ...
            <span className="text-gradient-gold glow-text-gold text-8xl">2026</span>
          </motion.h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
          ...
          delay: 0.5
            ...
            }} placeholder="✨ Enter Your Name ✨" maxLength={25} autoFocus className="w-full px-5 sm:px-6 py-4 sm:py-5 bg-muted/50 border-2 border-border/50 rounded-2xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all text-center text-lg sm:text-xl font-medium" />
          ...
          scale: 0.97
        }} className="w-full btn-celebration text-base sm:text-lg sm:py-5 flex items-center justify-center gap-3 px-[2px] py-[12px]">
        ...
        delay: 0.7
          ...
          text: "Shareable"
        }].map((item, i) => <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted/30 border border-border/30 text-xs text-muted-foreground">
              {item.icon}
              {item.text}
            </span>)}
        </motion.div>
